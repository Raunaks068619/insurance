...
- **Pure core committed** (`a11c081`); **everything since is uncommitted**: SQLite scaffolding +
  cycles **26–36** (writeback · status-transition log · first-class disputes) + the ERD GLOB `fix:` +
  a tree-wide `chore: format`. **46/46 green** (36/36 cycles), `tsc` + full `pnpm lint` clean.
  **The 36-cycle TDD plan is COMPLETE.** Next is NOT a cycle — the **HTTP/API layer**: submit claim,
  `GET /claims/:id` with `timeline` + `disputes[]` + `adjudication_history`, the dispute endpoint
  (`disputeService.open` already exists, with `DisputeError` → 404/409 mapping), seed data, run-through.
