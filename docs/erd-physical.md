...
### PHI
`members.name`/`dob`, `claims.provider`/`diagnosis_code` are PHI, isolated to member/claim records; the adjudication engine input is typed to exclude them (keys on `member_id` → policy + accumulators), so it **structurally** never reads them. SQLite has no native column encryption, so these four columns are **encrypted at rest via app-level field encryption** (AES-256-GCM, random 96-bit IV + 128-bit auth tag per value), implemented in [`app/src/db/phi-crypto.ts`](../app/src/db/phi-crypto.ts) and applied at a single persistence seam: the member/claim repositories encrypt on write and decrypt on read (`memberRepository.findMemberById`, `claimRepository.findClaimPhi`). The key is a 32-byte secret from `PHI_ENCRYPTION_KEY` (`.env`), with a documented DEV fallback so the demo runs. SQLCipher whole-file encryption is the production-scale alternative (transparent, encrypts indexes/WAL too) — deliberately not used here to avoid a native-driver swap; app-level field encryption keeps the stance visible and testable ([`app/tests/phi-encryption.test.ts`](../app/tests/phi-encryption.test.ts)).
...
- **Column encryption.** SQLite has no native column encryption. The 4 PHI columns are protected by **app-level field encryption (AES-256-GCM)** at the repository seam; SQLCipher whole-file encryption is the production alternative. Implemented and tested, not just documented.
